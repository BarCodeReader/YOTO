...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Sequential(
            act_layer(), ln_layer(self.dh, dim, bn_weight_init=0, resolution=resolution)
        )
    ...
    def forward(self, x):  # x (B,C,H,W)
        if self.use_conv:
            ...
            attn = q @ k.transpose(-2, -1) * self.scale + self.get_attention_biases(
                x.device
            )
            attn = attn.softmax(dim=-1)
...
class AttentionSubsample(nn.Module):
    ...
    def __init__(
        ...
        self.q = nn.Sequential(
            sub_layer(stride=stride), ln_layer(in_dim, nh_kd, resolution=resolution_)
        )
        self.proj = nn.Sequential(
            act_layer(), ln_layer(self.dh, out_dim, resolution=resolution_)
        )
    ...
    def forward(self, x):
        if self.use_conv:
            ...
            k, v = (
                self.kv(x)
                .view(B, self.num_heads, -1, H * W)
                .split([self.key_dim, self.d], dim=2)
            )
...
class Levit(nn.Module):
    ...
    def __init__(
        ...
        if distillation:
            self.head_dist = (
                NormLinear(embed_dim[-1], num_classes)
                if num_classes > 0
                else nn.Identity()
            )
